...
export default function CartSidebar() {
  ...
  const handleCheckout = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      let message = `Olá *MDA*, gostaria de um orçamento rápido dos produtos abaixo:%0A`;
    ...
    <AnimatePresence>
      {isSidebarOpen && (
        <>
          ...
          >
            {/* Header */}
            <div className="p-4 border-b border-earth-100 flex items-center justify-between bg-[#5A1788] text-white shrink-0">
                    ...
                    {!isCheckoutMode ? (
                      ...
                      <form onSubmit={handleCheckout} className="space-y-3">
                        ...
                        <button
                          type="submit"
                          disabled={isLoading}
                          className="w-full flex items-center justify-center gap-2 bg-[#5A1788] hover:bg-[#4a1270] disabled:bg-earth-300 disabled:cursor-not-allowed text-white px-6 py-3 rounded-xl text-lg font-bold transition-colors shadow-lg mt-2"
                        >
                          {isLoading ? (
                            <>
