        ...
        <div className="w-full animate-in fade-in duration-500">
            <main className="w-full">
                ...
                <div className="flex justify-between items-center mb-8">
                    ...
                    {activeTab === 'products' ? (
                        <button onClick={() => openProductModal()} className="flex items-center gap-2 bg-mustard-500 hover:bg-mustard-600 text-olive-900 px-4 py-2 rounded-xl font-bold shadow-md">
                            <Plus className="w-4 h-4" /> Novo Produto
                        </button>
                    ) : (
                        <button onClick={openCategoryModal} className="flex items-center gap-2 bg-mustard-500 hover:bg-mustard-600 text-olive-900 px-4 py-2 rounded-xl font-bold shadow-md">
                                ...
                                <tbody>
                                    {products.map(p => (
                                        <tr key={p.id} className={`border-b border-earth-100 hover:bg-earth-50 ${p.hidden === 1 ? 'opacity-60' : ''}`}>
                                            ...
                                            <td className="p-4 font-bold text-olive-900">{p.name}</td>
                                            <td className="p-4"><span className="bg-mustard-500/10 text-mustard-600 px-2 py-1 rounded text-xs font-bold uppercase">{categories.find(c => c.id === p.category)?.name || p.category}</span></td>
