...
const ArquivosPage: React.FC = () => {
  ...
  const mutation = useMutation({
    mutationFn: async ({ action, formData, id }: { action: 'insert' | 'update' | 'delete', formData?: Partial<Arquivo>, id?: string }) => {
      if (action === 'insert') {
        const { error } = await supabase.schema('gestaohashi').from('arquivos').insert(formData);
        if (error) throw error;
      } else if (action === 'update') {
        const { error } = await supabase.schema('gestaohashi').from('arquivos').update(formData).eq('id', id);
        if (error) throw error;
      } else if (action === 'delete') {
        const { error } = await supabase.schema('gestaohashi').from('arquivos').delete().eq('id', id);
        if (error) throw error;
      }
    },
    onSuccess: () => {
