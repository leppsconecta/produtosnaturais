...
const CurriculosPage: React.FC = () => {
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'mdaprodutosnaturais', table: 'curriculos' }, () => {
        refetch();
      })
      .subscribe();
          ...
          {item.anexo_url && (
            <div className="pt-4 border-t border-slate-100 dark:border-slate-700">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-2 px-4 py-2.5 bg-[#5A1788] hover:bg-[#4a1270] text-white font-semibold rounded-xl transition-all text-sm"
              >
                <Download size={16} />
                Baixar Currículo
              </a>
    ...
    },
    {
      ...
      accessor: (item: CurriculoRow) => (
        <button
          ...
          className="p-2 text-slate-500 hover:text-[#5A1788] hover:bg-purple-50 dark:hover:bg-purple-900/30 rounded-lg transition-all"
          title="Ver detalhes"
        >
          <Eye size={16} />
        </button>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-2 px-5 py-2.5 bg-[#5A1788] hover:bg-[#4a1270] text-white font-semibold rounded-xl transition-all shadow-sm hover:shadow-md text-sm"
        >
          Funções
        </button>
      </div>
